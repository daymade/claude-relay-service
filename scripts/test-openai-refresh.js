...
async function testRefreshToken() {
  ...
  try {
    ...
    console.log(
      '   Access Token:',
      result.access_token ? `${result.access_token.substring(0, 30)}...` : 'N/A'
    )
    console.log('   ID Token:', result.id_token ? `${result.id_token.substring(0, 30)}...` : 'N/A')
    console.log(
      '   Refresh Token:',
      result.refresh_token ? `${result.refresh_token.substring(0, 30)}...` : 'N/A'
    )
    ...
    if (process.env.PROXY_HOST && process.env.PROXY_PORT) {
      ...
      console.log(
        '   Access Token:',
        proxyResult.access_token ? `${proxyResult.access_token.substring(0, 30)}...` : 'N/A'
      )
    }
    ...
    if (process.env.OPENAI_ACCOUNT_ID) {
      ...
      try {
        ...
        if (account) {
          ...
          const refreshResult = await openaiAccountService.refreshAccountToken(
